...
const AnimatedButton = ({ active, onPress, label, image, color, activeTintColor, defaultTintColor, customIcon, buttonStyle, textStyle, labelStyle, disabled = false, maxWidth = 144, }) => {
    ...
    const boxPadding = useSharedValue(disabled
        ? 0
        : active
    ...
    useEffect(() => {
        if (textWidth) {
            if (disabled) {
                ...
                boxPadding.value = withTiming(active
                    ? (MAX_WIDTH - (textWidth + IMAGE_SIZE + TEXT_PADDING)) / 2 - INNER_BOX_PADDING_LEFT
                    : 0, { duration: 300 });
            }
        }
    ...
    return (<Pressable onPress={handlePress} disabled={disabled}>
      <Animated.View style={[styles.animatedBox, animatedStyle, buttonStyle]}>
        <Animated.View style={[styles.innerBox, animatedBoxStyle]}>
          <View style={styles.imageContainer}>
            {customIcon ? (customIcon) : (<Animated.Image source={image} style={[styles.image, { tintColor: iconTintColor }]} resizeMode="contain"/>)}
          </View>
